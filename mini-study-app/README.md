...
#### 2. 首页
- 系统公告弹窗：展示最新启用中的公告，仅在**每次登录后的首次进入首页时弹出一次**
- 轮播图展示
- 学习内容推荐
- 考试入口
- 学习进度概览
...
- 编辑内容信息
- 内容上线 / 下线：
  - 支持三种状态：`draft`（草稿）、`published`（已发布）、`offline`（已下线）
  - 学员端仅展示 `published` 状态的内容
  - 管理后台的内容列表支持一键在“已发布 / 已下线”之间切换
- 内容分类管理
...
#### 5. 公告管理
- 管理员在“小程序后台 → 公告管理”中维护系统公告
- 支持设置启用状态、图片、正文内容
- 列表按时间倒序展示（最新公告在最上方）
- 首页的系统公告弹窗会自动读取当前**时间窗口内、启用状态**且最新的一条公告
